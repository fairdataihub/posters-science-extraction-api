...
def load_json_model():
    ...
    if _json_model is None:
        ...
        _json_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct")
...
def extract_json_with_retry(raw_text: str, model, tokenizer) -> dict:
    ...
    if 'error' in result or is_truncated(result.get('raw', '')):
        log(f"   ⚠️ Still truncating, trying FALLBACK shorter prompt...")
...
def repair_unescaped_quotes(s: str) -> str:
    ...
    s = re.sub(r'(\d+\s*(?:pc|km|m|cm|mm|Hz|kHz|MHz|GHz|s|ms|ns|arcsec|arcmin|deg))/"', r'\1/\\"', s)
...
def calculate_forgiving_rouge(gen_text: str, ref_text: str, gen_sections: list, ref_sections: list) -> float:
...
def passes(m: dict) -> bool:
    return (m['word_capture'] >= 0.75 and m['rouge_l'] >= 0.75 and 
            m['number_capture'] >= 0.75 and 0.3 <= m['field_proportion'] <= 2.5)
...
def find_pairs(annotation_dir: str):
    ...
    for subdir in Path(annotation_dir).iterdir():
        ...
        poster_files = list(subdir.glob("*.pdf")) + list(subdir.glob("*.jpg")) + list(subdir.glob("*.png"))
...
def run(annotation_dir: str, output_dir: str):
    ...
    for i, (poster_path, ref_json_path, poster_id) in enumerate(pairs, 1):
        ...
        try:
            ...
            log(f"   w={m['word_capture']:.2f} r={m['rouge_l']:.2f} n={m['number_capture']:.2f} f={m['field_proportion']:.2f} {'✅' if p else '❌'}{err_str}{sec_str} [{time.time()-t0:.1f}s]")
    ...
    if successful:
        avg = {k: float(np.mean([r['metrics'][k] for r in successful])) for k in ['word_capture', 'rouge_l', 'number_capture', 'field_proportion']}
        passed = sum(1 for r in successful if r['passes'])
        errors = sum(1 for r in successful if r.get('error'))
        
        log(f"AVG: w={avg['word_capture']:.3f} r={avg['rouge_l']:.3f} n={avg['number_capture']:.3f} f={avg['field_proportion']:.3f}")
        ...
        for r in successful:
            ...
            m = r['metrics']
            src = r.get('source', '?')
            log(f"  {s} {r['poster_id']}: w={m['word_capture']:.2f} r={m['rouge_l']:.2f} n={m['number_capture']:.2f} f={m['field_proportion']:.2f} sec:{m['gen_sections']}/{m['ref_sections']}{e} [{src}]")
